...
router.get("/*", function(req, res) {
	...
	db.WorkOrder.findById(req.params[0], function (err, workorder) {
		...
		else if(typeof workorder !== "undefined" && workorder !== null) res.send(workorder).status(200);
		else if(workorder === null) {
			db.Customer.findById(req.params[0], function (err, customer) {
				if (err) return console.log(err);
				else if (customer) {
					db.WorkOrder.find({customerID: customer}).then(function(workorders) {res.send(workorders).status(200)});
				}
			});
		}
	}).catch(function(err){
