...
router.get("/all", function(req, res) {
	...
	db.Cost.find({}).then( function(costs) {res.send(costs).status(200);});
});
...
router.delete("/*", function(req, res) {
	db.Cost.findById(req.params[0], function (err, cost) { if(cost) cost.remove();}).then(function (cost){res.send(cost).status(200)});
});
...
router.put("/*", function(req, res) {
	...
	db.Cost.update({_id:req.params[0]}, req.body, function(err, numAffected){
