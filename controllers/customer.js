...
router.get("/all", function(req, res) {
	...
	db.Customer.find({}).then( function(customers) {res.send(customers).status(200);});
});
...
router.delete("/*", function(req, res) {
	db.Customer.findById(req.params[0], function (err, customer) { if(customer) customer.remove();}).then(function (customer){res.send(customer).status(200)});
});
...
router.put("/*", function(req, res) {
	...
	db.Customer.update({_id:req.params[0]}, req.body, function(err, numAffected){
